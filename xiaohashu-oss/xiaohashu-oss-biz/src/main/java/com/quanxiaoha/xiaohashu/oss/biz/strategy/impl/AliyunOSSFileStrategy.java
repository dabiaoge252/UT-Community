...
public class AliyunOSSFileStrategy implements FileStrategy  {
    ...
    public String uploadFile(MultipartFile file) {
        ...
        // 上传文件至阿里云 OSS（bucket 名称从配置读取，按存储方式动态选择）
        ossClient.putObject(aliyunOSSProperties.getBucket(), objectName, new ByteArrayInputStream(file.getInputStream().readAllBytes()));

        // 返回文件的访问链接
        String url = String.format("https://%s.%s/%s", aliyunOSSProperties.getBucket(), aliyunOSSProperties.getEndpoint(), objectName);
