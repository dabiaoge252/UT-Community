...
/*      创建并配置一个用于在Redis服务端执行Lua脚本的脚本对象。
        <Long> 指定了这段Lua脚本执行完毕后返回值的Java类型。*/
        ...
        if (Objects.equals(result, LuaResultEnum.ZSET_NOT_EXIST.getCode())) {
            ...
            // 若记录为空（用户没有关注过人），直接 ZADD 对象, 并设置过期时间
    ...
    public PageResponse<FindFollowingUserRspVO> findFollowingList(FindFollowingListReqVO findFollowingListReqVO) {
        ...
        if (total > 0) { // 缓存中有数据
            ...
            // 注意：这里使用了 Double.POSITIVE_INFINITY 和 Double.NEGATIVE_INFINITY 作为分数范围，正无穷和负无穷
            ...
            //rangeByScore 是按分数从小到大排，而 reverse 是按分数从大到小排。
